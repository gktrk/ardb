...
/*
** Windows CE helper routine.
** Similar to strdup, but first converts the ANSI string to UNICODE
** and then returns the UNICODE clone.
*/
#ifdef _WIN32_WCE
static WCHAR * wStrDup( char const * str )
{
	size_t len = strlen(str) + 1; // +1 for terminating '\0'
	WCHAR * aux = (WCHAR *) sqliteMalloc( len*sizeof(WCHAR) );
	mbstowcs( aux, str, len );
	return aux;
}
#endif
...
/*
** The same applies for the assert() routine.
*/
...
/* --------------------------------------------------------------------- */
/*
** WinCE File Locking routines.
**
** These are just helpers for the sqliteOsFileLock family of functions.
** The idea is to have a named counter, made with a named mutex and a named
** semaphore. The name of this resources is generated from the file path, so
** that they are unique for each database opened.
**
** I'm sure there are many other ways to acomplish the same thing, and maybe
** some are better. If you know any better way, just say so, and I will be
** happy to use it.
*/
#if defined(_WIN32_WCE) && ! defined(SQLITE_WCE_OMIT_FILELOCK)

// I think this is more than enough for Windows CE
#define MAX_FILE_READERS 64

static BOOL wceport_OpenFileLock( OsFile * id )
{
	WCHAR aux[256];
	WCHAR muxName[256] = L"SqliteMux";
	WCHAR semName[256] = L"SqliteSem";
	int i = 0, j = 0;
	for ( ; id->filePath[i]; ++i )
		switch ( id->filePath[i] )
		{
		case '\\':
		case '/':		break;
		default:		aux[j] = id->filePath[i];
						++j;
		}
	aux[j] = '\0';
	wcscat( muxName, aux );
	id->hMux = CreateMutexW( NULL, FALSE, muxName );
	if ( id->hMux == NULL )
		return FALSE;
	wcscat( semName, aux );
	id->hSem = CreateSemaphoreW( NULL, MAX_FILE_READERS,
								MAX_FILE_READERS, semName );
	if ( id->hSem == 0 )
	{
		CloseHandle( id->hMux );
		return FALSE;
...
static void wceport_CloseFileLock( OsFile * id )
{
	if ( id->hMux )
		CloseHandle( id->hMux );
	if ( id->hSem )
...
// Lock access to file by all processes.
// Returns TRUE on success, FALSE on failure.
static BOOL wceport_LockFileMutex( HANDLE hMux )
{
	DWORD res = WaitForSingleObject( hMux, 250 );
	...
	{
		ReleaseMutex( hMux );
		res = WaitForSingleObject( hMux, 250 );
	}
	// success ?
	if ( res != WAIT_OBJECT_0 )
	{
		return FALSE;	// someone is taking a long time
	}
	return TRUE;
}
static int wceport_IncrementSemaphoreN( HANDLE hSem, unsigned count )
{
	DWORD prevCount;
	BOOL res = ReleaseSemaphore( hSem, count, &prevCount );
	// this can only fail on a program bug or operating sistem error
	// what should we do if that happens ?
	assert( res != FALSE );
	return prevCount;
}
static BOOL wceport_DecrementSemaphore( HANDLE hSem )
{
	DWORD res = WaitForSingleObject( hSem, 250 );
	// I don't know very well what I have to do in this case.
	// Does this apply to semaphores ??!!...
//	if ( res == WAIT_ABANDONED )
//		....
	// success ?
	if ( res != WAIT_OBJECT_0 )
...
// Decrement semaphore by a specified amount.
// If failure, restore the original semaphore count.
static BOOL wceport_DecrementSemaphoreN( HANDLE hSem, unsigned count )
{
	unsigned i;
	for ( i = 0; i < count; ++i )
	{
		if ( ! wceport_DecrementSemaphore(hSem) )
		{	// Restore original count
			if ( i > 0 )
				wceport_IncrementSemaphoreN( hSem, i );
			return FALSE;
		}
	}
	return TRUE;
...
#endif // UNDER_CE && ! SQLITE_WCE_OMIT_FILELOCK
/* --------------------------------------------------------------------- */
...
# else
  wchar_t * aux = wStrDup( zFilename );
  int ret = GetFileAttributesW( aux ) != 0xFFFFFFFF;
  free( aux );
  return ret;
# endif
#endif
...
# ifndef _WIN32_WCE
  ...
  id->locked = 0;
# else
  WCHAR * wFilename = wStrDup( zFilename );
  // Try to open for read/write
  HANDLE h = CreateFileW( wFilename, GENERIC_READ | GENERIC_WRITE,
      FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS,
      FILE_ATTRIBUTE_NORMAL | FILE_FLAG_RANDOM_ACCESS, NULL );
  // If it fails, try to open read-only
  if ( h==INVALID_HANDLE_VALUE )
  {
    h = CreateFileW( wFilename, GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  id->h = h;
  id->delOnClose = 0;
  id->filePath = wFilename;
#  ifndef SQLITE_WCE_OMIT_FILELOCK
  // Prepare file lock mechanism
  if ( ! wceport_OpenFileLock( id ) )
  {
     // Failled to initialize file lock mechanism
     CloseHandle( id->h );
     /* NOTE: the spec says that *id and *pReadOnly should be left
              unchanged on failure, but I don't see any problem on
              ignoring this here. Should this be fixed ? */
     return SQLITE_CANTOPEN;
  }
#  else // SQLITE_OMIT_FILELOCK
  id->hMux   = 0;
  id->hSem   = 0;
  id->locked = 0;
#  endif // SQLITE_OMIT_FILELOCK
# endif
  OpenCounter(+1);
  return SQLITE_OK;
#endif
...
# else
  WCHAR * wFilename = wStrDup( zFilename );
  HANDLE h = CreateFileW( wFilename, GENERIC_READ | GENERIC_WRITE, 0,
  ...
  id->filePath = wFilename;
  id->hMux   = 0;	// Not shared, so no need to lock file
  id->hSem   = 0;
  id->locked = 0;
# endif
  OpenCounter(+1);
...
# else
  WCHAR * wFilename = wStrDup( zFilename );
  HANDLE h = CreateFileW( wFilename, GENERIC_READ, 0, NULL, OPEN_EXISTING,
               FILE_ATTRIBUTE_NORMAL | FILE_FLAG_RANDOM_ACCESS, NULL );
  if ( h == INVALID_HANDLE_VALUE )
  {
    free( wFilename );
...
# else
    {
      wchar_t wcs[SQLITE_TEMPNAME_SIZE - 30];
      GetTempPathW( SQLITE_TEMPNAME_SIZE - 30, wcs );
      wcstombs( zTempPath, wcs, SQLITE_TEMPNAME_SIZE - 30 );
    }
# endif
...
# else
  ...
  CloseHandle( id->h );
#  ifndef SQLITE_WCE_OMIT_FILELOCK
  wceport_CloseFileLock( id );
#  endif // SQLITE_OMIT_FILELOCK
  if ( id->delOnClose )
  {
    DeleteFileW( id->filePath );
  }
  free( id->filePath );
# endif
  OpenCounter(-1);
...
# ifndef _WIN32_WCE
  ...
  return rc;
# else
  // if already read locked, return success
  if ( id->locked > 0 )
    return SQLITE_OK;
  // If file not shared, always success
  if ( id->hMux == 0 )
  {
     id->locked = 1;
     return SQLITE_OK;
...
#  ifndef SQLITE_WCE_OMIT_FILELOCK
  // Lock file for all processes
  if ( ! wceport_LockFileMutex(id->hMux) )
     return SQLITE_BUSY;
  // if file is write locked, release semaphore to a single reader state
  if ( id->locked < 0 )
  {
     BOOL res = ReleaseSemaphore( id->hSem, MAX_FILE_READERS-1, NULL );
     // Update lock status and release file mutex lock
     id->locked = 1;
  ...
  // not locked yet, try to decrement semaphore
  if ( ! wceport_DecrementSemaphore(id->hSem) )
  {	// Write-locked by other thread, or maximum readers reached
     ReleaseMutex( id->hMux );
     return SQLITE_BUSY;
  }
  id->locked = 1;
...
# else
  int decCount = 0;
  // if already write locked, return success
  if ( id->locked < 0 )
     return SQLITE_OK;
  // If file not shared, always success
  if ( id->hMux == 0 )
...
#  ifndef SQLITE_WCE_OMIT_FILELOCK
  ...
  if ( ! wceport_LockFileMutex(id->hMux) )
    return SQLITE_BUSY;
  // If I have a read-lock, release lock
  if ( id->locked > 0 && sqliteOsUnlock(id) != SQLITE_OK )
  {
    ReleaseSemaphore( id->hSem, 1, NULL );
    id->locked = 0;
  }
  // try to decrement semaphore to non-signaled state
  decCount = id->locked > 0 ? MAX_FILE_READERS - 1 : MAX_FILE_READERS;
  if ( ! wceport_DecrementSemaphoreN(id->hSem,decCount) )
  {	// Someone is locking the file (reader or writer)
    ReleaseMutex( id->hMux );
    return SQLITE_BUSY;
  }
  // success
...
#  ifndef SQLITE_WCE_OMIT_FILELOCK
  // Lock file for all processes
  if ( ! wceport_LockFileMutex(id->hMux) )
    return SQLITE_BUSY;	// This cannot happen
  if ( id->locked < 0 )
  {	// Write locked
    wceport_IncrementSemaphoreN( id->hSem, MAX_FILE_READERS );
  }
  else
  {	// Read locked
    wceport_IncrementSemaphoreN( id->hSem, 1 );
  }
  // Update lock status and release file mutex lock
  id->locked = 0;
  ReleaseMutex( id->hMux );
...
# else
  // There is no current directory on Windows CE, so we assume
  // we are working always with absolute paths
  char *zFull = 0;
...
#else
  {
	  /* WinCE doesn't have GetSystemTimeAsFileTime() */
	  SYSTEMTIME st;
	  GetSystemTime( &st );
