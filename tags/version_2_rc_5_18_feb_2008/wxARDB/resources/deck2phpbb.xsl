...
<xsl:call-template name="count-vampires"><xsl:with-param name="myname" select="string(name)" /><xsl:with-param name="myadv" select="string(adv)" /></xsl:call-template><xsl:text>x [url=http://monger.vekn.org/showvamp.html?NAME=</xsl:text><xsl:call-template name="url-encode"><xsl:with-param name="str" select="string(name)"/></xsl:call-template><xsl:if test="adv!=''">%20ADV</xsl:if><xsl:text>] </xsl:text><xsl:value-of select="string(name)"/><xsl:choose><xsl:when test="adv != ''"><xsl:text> Adv </xsl:text></xsl:when><xsl:otherwise><xsl:text>     </xsl:text></xsl:otherwise></xsl:choose><xsl:text> [/url] (</xsl:text><xsl:value-of select="capacity"/><xsl:text>) </xsl:text><xsl:call-template name="str:tokenize"><xsl:with-param name="string" select="string(disciplines)" /><xsl:with-param name="delimiters" select="' '"/></xsl:call-template><xsl:text> </xsl:text><xsl:value-of select="string(title)"/><xsl:text> </xsl:text><xsl:value-of select="string(clan)"/> (group <xsl:value-of select="group"/><xsl:text>)&#013;&#010;</xsl:text>
</xsl:for-each>
</xsl:for-each>
...
<xsl:sort select="name"/>
<xsl:text>  </xsl:text><xsl:call-template name="count-cards"><xsl:with-param name="myname" select="string(name)" /></xsl:call-template><xsl:text>x [url=http://monger.vekn.org/showcard.html?NAME=</xsl:text><xsl:call-template name="url-encode"><xsl:with-param name="str" select="string(name)"/></xsl:call-template><xsl:text>] </xsl:text><xsl:value-of select="string(name)"/><xsl:text> [/url]&#013;&#010;</xsl:text>
